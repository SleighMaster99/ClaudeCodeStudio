...
- For key technology choices: verify the specified versions actually exist (e.g., check npm registry, official docs)
- For CLI tools specified: run `--help` to confirm the tool and commands exist
- For API integrations specified: verify the documented API endpoints/signatures against official documentation
- For starter templates or project generators: verify the exact command works
- Tag any unverifiable claims (e.g., future API features, unreleased versions) and note for downstream verification
