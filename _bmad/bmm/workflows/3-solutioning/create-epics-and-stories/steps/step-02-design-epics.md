...
## 🔀 PARALLEL DEVELOPMENT RULES (병렬 개발 규칙)

각 에픽은 다른 개발자가 동시에 개발하고 독립적으로 테스트할 수 있어야 합니다.

### 허용되는 의존성
- **Epic 1 (Foundation)에 대한 의존성**: 프로젝트 기본 파일, 공통 설정, 기본 인터페이스
- 예: .uproject, .sln, 프로젝트 구조, 공통 유틸리티

### 1. 기능적 독립성
- 각 에픽은 다른 기능 에픽 없이 **독립적으로 동작/테스트 가능**
- Epic 2는 Epic 3 구현 없이 테스트 가능해야 함
- 다른 에픽의 기능이 필요하면 **모킹/스텁** 사용

### 2. 머지 충돌 방지 설계
- 같은 파일 수정 가능하지만, **머지 시 충돌 없도록 설계**
- 가능하면 다른 섹션/함수 수정
- 충돌 예상 시 **작업 순서 또는 인터페이스로 분리**

### 3. 인터페이스 기반 협력
- 에픽 간 협력이 필요하면 **인터페이스/계약** 정의
- Epic 1에서 공통 인터페이스 정의 권장
- 구현은 각 에픽에서 독립적으로

### 4. 독립 테스트 전략
- 각 에픽은 자체 테스트 가능해야 함
- 다른 에픽 의존 시 모킹으로 대체
- 통합 테스트는 모든 에픽 머지 후 실행
...
**File:** [epics/epic-1.md](epics/epic-1.md)

### Epic 2: [Epic Title]
[Epic goal statement - what users can accomplish]
**FRs covered:** FR4, FR5, FR6, etc.
**File:** [epics/epic-2.md](epics/epic-2.md)

[Continue for all epics]
```

**NOTE:** The **File** line records where each epic's detailed content (goal, FR coverage, stories) will be written. The epic files themselves are created in the next step (Step 3) under `{epics_artifacts}` - do NOT create them now.
