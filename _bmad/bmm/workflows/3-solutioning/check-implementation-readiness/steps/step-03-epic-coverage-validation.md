...
- Load the epics index (`epics.md`) AND every epic detail file linked from its Epic List (or the legacy single document if that is what exists)
- Read them completely to find FR coverage information
- Look for sections like "FR Coverage Map" or similar (in the index)
