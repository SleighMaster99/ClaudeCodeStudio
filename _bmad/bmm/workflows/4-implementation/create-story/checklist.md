...
- Load the epic file `{epics_artifacts}/epic-{{epic_num}}.md` (or `{epics_file}` / sharded equivalents for legacy structures)
...
#### **2.6 Technical Claims Verification (MANDATORY)**

🔍 **Every technical spec in the story MUST be verified against reality — not just researched.**

- [ ] **CLI Commands**: All CLI commands in Dev Notes have been verified with `--help` (actual output confirms the command, subcommands, and flags exist)
- [ ] **External Package APIs**: Function signatures, parameter types, and return types have been confirmed against actual type definitions or official documentation (not hallucinated or guessed)
- [ ] **File Paths/Structure**: All referenced file paths and directory structures have been confirmed to exist in the actual project via `ls`/`cat`/`read`
- [ ] **Internal Code References**: All referenced functions, types, constants, and modules have been confirmed to exist in the codebase with correct signatures via `grep`/`read`
- [ ] **Unverified Items Tagged**: Any claims that cannot be verified at story-creation time are tagged `[unverified]` with explicit Dev Agent verification tasks added
- [ ] **Code Examples Validated**: Dev Notes code examples are based on confirmed APIs, not assumptions or approximations
- [ ] **No Speculative Specs**: Story contains zero untagged speculative technical claims
...
### **Step 3.6: Story 문서 구조 원칙 준수 검증 (MANDATORY)**

🎯 **Story / Acceptance Criteria / Tasks·Subtasks 섹션은 요점만, 상세는 Dev Notes에 모은다는 원칙을 반드시 검증한다.**

- [ ] **Story 섹션 간결성**: 역할/원하는 행동/얻을 가치를 각 한 줄로 작성했는가? 배경·동기·맥락 설명이 본문에 섞여 있지 않은가?
- [ ] **Acceptance Criteria 간결성**: 각 AC가 검증 가능한 결과를 한 줄로 표현했는가? 구현 방법, 코드 예시, 기술 상세가 AC에 포함되어 있지 않은가?
- [ ] **Tasks·Subtasks 간결성**: 각 task가 짧은 동사구로 "무엇을 할지"만 표현하는가? 코드 스니펫, API 시그니처, 파일 경로 상세, 라이브러리 사용법 등이 task 본문이 아닌 Dev Notes에 있는가?
- [ ] **Dev Notes 집결**: 배경, 기술적 결정, 코드 예시, API 시그니처, 파일 경로 상세, 라이브러리 버전, 제약, 출처 인용 등 모든 상세 내용이 Dev Notes(및 하위 섹션)에 모여 있는가?
- [ ] **중복 없음**: 동일 정보가 Story/AC/Tasks와 Dev Notes 양쪽에 중복 기재되어 있지 않은가? (상세는 Dev Notes에만)
- [ ] **참조 연결**: Tasks에서 "어떻게 할지"가 필요한 경우 본문에 직접 쓰지 않고 "→ Dev Notes의 X 참조"로 연결되어 있는가?

**위반 발견 시 Critical Issue로 분류하고 Step 5에서 상세 내용을 Dev Notes로 이동시키는 개선안을 제시한다.**
