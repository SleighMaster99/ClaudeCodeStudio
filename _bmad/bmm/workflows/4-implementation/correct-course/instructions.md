...
<action>Verify technical claims in all change proposals:
  For EACH proposed change that includes CLI commands, API signatures, file paths, or internal code references:
  1. CLI commands → run `--help` to confirm existence and correct flags
  2. API signatures → check actual type definitions or documentation
  3. File paths → run `ls`/`read` to confirm existence
  4. Internal code references → use `grep`/`read` to confirm existence and signatures
  5. If a claim cannot be verified → tag as [unverified] and note that Dev Agent must verify before implementing
  6. If a claim is incorrect → fix the proposal to match reality and document the correction
</action>
