...
### Technical Claims Verification (MANDATORY):

Before marking spec as ready for review, verify ALL technical claims:

- [ ] **CLI Commands:** All CLI commands in tasks have been verified with `--help` (confirmed to exist with correct flags)
- [ ] **API Signatures:** All external package API function signatures have been confirmed against actual type definitions or documentation
- [ ] **File Paths:** All referenced file paths/directories have been confirmed to exist via `ls`/`read`
- [ ] **Internal Code References:** All referenced functions, types, and modules have been confirmed via `grep`/`read`
- [ ] **Unverified Items Tagged:** Any claims that cannot be verified are tagged `[unverified]` with a note for the dev to verify before implementing
- [ ] **No Speculative Specs:** Tech-spec contains zero untagged speculative technical claims
